...
class NewsItem {
  ...
  static String _getText(XmlElement item, String tagName) {
    try {
      final element = item.findElements(tagName).first;
      var text = element.text;
      // Decode CDATA if present and trim
      return text.replaceAll(RegExp(r'<!\[CDATA\[|\]\]>'), '').trim();
    } catch (e) {
      return '';
    }
  }
  ...
  static DateTime _parseDate(String dateStr) {
    try {
      ...
      try {
        return DateTime.parse(dateStr.replaceAll(RegExp(r' [+-]\d{4}$'), 'Z'));
      } catch (_) {
        return DateTime.now();
      }
    }
  ...
  static String? _extractImageUrl(XmlElement item) {
    try {
      ...
      // Atom feeds <content:encoded> or <content>
      final contentEncoded = item.findElements('content:encoded');
      if (contentEncoded.isNotEmpty) {
        final match = RegExp(r'src="(.*?)"').firstMatch(contentEncoded.first.text);
...
class RSSFeedService {
  ...
  static Future<List<NewsItem>> fetchRSSFeed(String url, String source) async {
    try {
      ...
      if (response.statusCode == 200) {
        final body = utf8.decode(response.bodyBytes); // UTF-8 decode
        final document = XmlDocument.parse(body);

        final items = document.findAllElements('item');
        final entries = document.findAllElements('entry'); // For Atom feeds

        final allItems = [
          ...items.map((item) => NewsItem.fromXml(item, source)),
          ...entries.map((entry) => NewsItem.fromXml(entry, source)),
        ];

        return allItems.where((item) => item.title.isNotEmpty && item.link.isNotEmpty).toList();
      }
  ...
  static Future<List<NewsItem>> aggregateNews() async {
    ...
    final futures = bangladeshiFeeds.entries
        .map((entry) => fetchRSSFeed(entry.key, entry.value));
